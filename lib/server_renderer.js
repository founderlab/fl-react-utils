...
function createServerRenderer(options) {
  ...
  return function app(req, res) {
    ...
    store.dispatch((0, _reduxRouterServer.match)(req.originalUrl, function (err, redirect_location, router_state) {
      ...
      (0, _fetch_component_data2['default'])({ store: store, components: router_state.components }, function (err) {
        if (err) {
          console.log(err);
          return res.status(500).send('Internal server error');
        }
